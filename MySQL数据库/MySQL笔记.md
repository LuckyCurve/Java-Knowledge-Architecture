...
如果此时要查询order\_date为2000-09-01的订单，使用以下SQL语句
...
每个表只允许一个AUTO\_INCREMENT列，并且该列要被索引（使他成为主键）
...
将检索出的第一行的order\_num列插入到一个局部变量o中
...
-   mysql\> CHECK TABLE orders,roles;

针对许多问题对表进行检查

![image-20200326203136943](images/image-20200326203136943.png)

-   如果使用CHECK TABLE检查出的MyISAM表出现错误，可能需要使用REPAIR
    TABLE
    来解决问题（不应该经常用，如果经常使用这说明有更大的问题需要解决）

-   如果从一个表删除大量数据，应该使用OPTIMIZE TABLE来收回所用的空间
...
-   --verbose 显示全文本消息，一般与--help联合使用
-   -- version显示版本信息

查看日志
--------

-   错误日志：包含启动和关闭问题以及关键错误的细节，在data目录，常被命名为hostname.err
-   查询日志：记录所有MySQL活动，常用于整断问题，在data目录中，常命名为hostname.log
-   二进制日志：记录更新过数据（可能更新过数据）的所有语句，位于data目录下，一般名为hostname-bin
-   缓慢查询日志：记录执行缓慢的任何查询，对优化很有用，位于data目录下

基于MySQL5，可能不兼容与MySQL8

三十、改善性能
==============

数据库管理员将大量时间花在了调整，优化DBMS性能上

以下是一些对性能优化的探讨和分析：

-   遵循MySQL的硬件建议
-   运行在专用服务器上
-   可能要调整MySQL的默认配置以适应现实
-   MySQL是一个多用户多线程的程序，当某一进程执行缓慢，则其他线程也会受到影响，可以使用SHOW
    PROCESSLIST，然后使用KILL杀死某个缓慢的进程
-   找出最佳的办法写SELECT语句
-   使用存储过程代替一条一条执行MySQL语句
-   使用正确的数据类型
-   绝不检索比需求还多的数据（不要使用SELECT \*
    除非你真正需要所有数据的话）
-   对于有些操作（INSERT）可以使用DELAYED关键字，立即返回结果给客户端，操作等有空的时候再进行（类似于MQ的作用）
-   导入数据时候应该关闭自动提交，导入完成后再手动提交，可能还要删除索引（如果数据量太大的话）等导入完成后再建立索引
-   必须索引数据库表以改善检索性能

![image-20200324184528289](images/image-20200324184528289.png)

-   使用多条SELECT语句用UNION链接代替使用一大堆的OR条件，性能会快上很多
-   索引虽然可以改善检索的性能，但会损害插入，删除和更新的性能
-   LIKE很慢，最好使用FULLTEXT而不是LIKE
-   随着数据的增加，刚优化好的表可能又需要重新优化
-   **最重要的规则就是：每条规则在某条数据的插入下都会被打破**
