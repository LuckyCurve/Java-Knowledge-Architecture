...
- 如果使用CHECK TABLE检查出的MyISAM表出现错误，可能需要使用REPAIR TABLE 来解决问题（不应该经常用，如果经常使用这说明有更大的问题需要解决）
...
- MySQL是一个多用户多线程的程序，当某一进程执行缓慢，则其他线程也会受到影响，可以使用SHOW  PROCESSLIST，然后使用KILL杀死某个缓慢的进程
- 找出最佳的办法写SELECT语句
- 使用存储过程代替一条一条执行MySQL语句
- 使用正确的数据类型
- 绝不检索比需求还多的数据（不要使用SELECT * 除非你真正需要所有数据的话）
